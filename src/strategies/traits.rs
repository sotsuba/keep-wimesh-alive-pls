        ...
        match response {
            Ok(res) => {
                ...
                if !final_url.contains("neverssl.com") {
                    tracing::info!("Session appears to be dead; final URL after probe was: {}", final_url);
                    false
                } else {
                    tracing::info!("Session appears to be alive; final URL after probe was: {}", final_url);
                    true
                }
            }
            Err(_) => false,
...
mod tests {
    ...
    async fn test_real_environment_session_alive() {
        ...
        assert!(is_alive, "Session should be alive if you have internet access and are authenticated");
    }
}